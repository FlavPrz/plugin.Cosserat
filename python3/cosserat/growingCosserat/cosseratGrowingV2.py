...
class controlGrowings(Sofa.Core.Controller):
    def __init__(self, *args, **kwargs):
        Sofa.Core.Controller.__init__(self, *args, **kwargs)
        self.cosserat = kwargs['cosserat']
        self.index = self.oldIndex = 2

    def onAnimateBeginEvent(self, event):
        for i in range(10):
            cosseratCoordNode_i = getattr(self.cosserat, 'cosseratCoordinate'+str(i)+'Node')
            # print(f' ===> {cosseratCoordNode_i.cosseratCoordinateMO.position.value}')
            with cosseratCoordNode_i.cosseratCoordinateMO.position.writeable() as positionS:
                # print(f'index :{positionS}')
                positionSM = []
...
class Cosserat(Sofa.Prefab):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print(f'c est quoi SM : {positionSM}' )
    ...
    def addCosseratCoordinate(self, positionS, longeurS):
        ...
        for i in range (10):
            ...
            cosseratCoordinateNode_i.addObject('MechanicalObject', template='Vec3d',
                                               name='cosseratCoordinateMO', position=positionS,
                                               showIndices=0)
...
def createScene(rootNode):
    ...
    #     print(" ++++++++++++++++++++++ : " +target.name)
    #
    # animate(my_animation, {"target": rootNode}, 10)

    nonLinearCosserat = rootNode.addObject(
        controlGrowings(parent=rootNode, cosserat=rootNode))
    #control = rootNode.addChild(ForceController())
